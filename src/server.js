...
app.post('/upload', upload.single('myImage'), (req, res) => {
  client
    ...
    .then((results) => {
      ...
      const identification = { breed: webDetection.webEntities[0].description };
...
app.post('/uploadText', upload.single('myImage'), (req, res) => {
  client
    .textDetection(req.file.buffer)
    .then((results) => {
      console.log(results);
      const textDetection = results[0].fullTextAnnotation;
      const identificationT = { breed: textDetection.text };
